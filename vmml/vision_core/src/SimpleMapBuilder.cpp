...
SimpleMapBuilder::TmpFrame::track(const kfid &kf)
{
	...
	if (isMoving==false or matchesToKeyFrame.size()<10) {
		this->setPose(parentKeyFrame->pose());
		return false;
	}
	...
	cout << "Tracking; total: " << matchesToKeyFrame.size() << "; map: " << prevMapPointPairs.size() << "; candidates: " << candidatesMapPointPairs.size() << endl;
